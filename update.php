...
if (isset($_GET['id'])) {  
    ...
    $db->bind("id",$_GET['id']);
    $contact = $db->row('SELECT * FROM contacts WHERE id = :id');
...
<div class="content update">
    ...
    <form action="update.php?id=<?=$contact['id']?>&page=<?=$page?>" method="post">
        ...
        <label for="created">Created (readonly)</label>
        <input type="text" name="title" placeholder="Employee" value="<?=$contact['title']?>" id="title">
        <input type="datetime-local" name="created" value="<?=date('Y-m-d H:i', strtotime($contact['created']))?>" id="created" readonly>
        <input type="submit" value="Update">
        <a href="./read.php?page=<?=$page?>" class="btn btn-success">List</a>
    </form>
    <?php if ($msg): ?>
    <p><?=$msg?></p>
    <?php endif; ?>
